...
int main() {
	...
	for (x = A; x <= B; x += k) {  //цикл для выч. значения функ-и "у"
		y = sin(x);
		summn = x; // присвоение для того, чтобы пропустить начальный шаг, при i=0

		i = 1;
		do //цикл для выч. суммы с точностью при n шагов
		{
			summn += pow(-1, i) * pow(x, 2 * i + 1) / fact(2 * i + 1); // вычисление суммы
			i++;
		} while (i <= n);

		summe = x; // присвоение для того, чтобы пропустить начальный шаг, при i=0
		j = 1;
		do //цикл для выч. суммы с точностью при эпсилон
		{

			summe += pow(-1, j) * pow(x, 2 * j + 1) / fact(2 * j + 1); // вычисление суммы
			j++;
